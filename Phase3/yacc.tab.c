...
{
       0,    92,    92,    92,   106,   106,   123,   124,   128,   132,
     133,   134,   135,   140,   142,   140,   157,   177,   196,   220,
     240,   262,   266,   270,   277,   293,   308,   329,   349,   367,
     376,   377,   378,   381,   383,   381,   398,   418,   437,   461,
     481,   503,   521,   540,   559,   582,   582,   583,   584,   588,
     589,   590,   591,   592,   593,   594,   595,   596,   597,   598,
     599,   600,   604,   605,   610,   621,   622,   623,   624,   625,
     626,   627,   632,   653,   674,   701,   725,   729,   733,   737,
     745,   749,   750,   751,   752,   753,   757,   758,   765,   768,
     771,   774,   780,   787,   794,   801,   808,   814,   817,   820,
     823,   826,   829,   833,   840,   847,   858,   865,   877,   885,
     886,   887,   888,   889,   890,   891,   892,   896,   907,   907,
     908,   912,   918,   927,   927,   928,   932,   941,   945,   958,
     979,   986,   993,  1000,  1007,  1023,  1027,  1031,  1043,  1049,
    1052,  1059,  1067,  1075,  1078,  1084,  1093,  1103,  1108,  1116,
    1121,  1126,  1131,  1145,  1163,  1164,  1168,  1169,  1173,  1174,
...
#line 277 "yacc.y"
        ...
        //check if id name is in params already
        if(lookup((yyval.functions).params, (yyval.functions).params, entry->name) != NULL)
        {
...
#line 1000 "yacc.y"
        ...
        if(return_flag == 0)
        {
            printf("Error: Return statement not allowed\n");
            YYABORT;
        }
    }
#line 2732 "yacc.tab.c"
    break;
...
#line 1007 "yacc.y"
        ...
        if((yyvsp[-1].type) != 0 && (yyvsp[-1].type) != 1 && (yyvsp[-1].type) != 2 && (yyvsp[-1].type) != 3)
        {
...
#line 1093 "yacc.y"
                     {
        struct idrec *entry = lookup(global_table, global_table, (yyvsp[-1].id).name);
        if(entry == NULL){
            printf("Error: Variable %s not declared\n", (yyvsp[-1].id).name);
            YYABORT;
        }
